...
app.get('/:countryName/:countryCode', async (req, res) => {
    ...
    try {
        ...
        const borderCountries = borderResponse.data.borders?.filter((border: { borders: null; }) => border.borders !== null);
        const populationData = populationResponse.data.data?.filter((country: { country: string; }) => country.country == countryName);
        const flagURL;
    } catch (error) {
